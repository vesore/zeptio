...
const ROLE_HINTS: Record<Role, string> = {
  ...
  'Teacher':   'Clear explanations. Relatable examples.',
  'Child':     'Simple vocabulary. Curious and honest.',
  'CEO':       'High-level thinking. Results and impact.',
  'Creative':  'Unconventional angles. Imaginative output.',
}
...
export default function RoleAssignment({
  ...
  // Pre-fill textarea when role is selected
  useEffect(() => {
    if (selectedRole && result === null) {
      setPrompt(`You are a ${selectedRole}. `)
    }
  }, [selectedRole, result])
  ...
  async function handleSubmit() {
    if (!selectedRole || !prompt.trim() || isLoading) return
    // Ensure the role is present in the prompt
    const hasRole = prompt.toLowerCase().startsWith(`you are a ${selectedRole.toLowerCase()}`)
    const finalPrompt = hasRole ? prompt : `You are a ${selectedRole}. ${prompt}`
  ...
  function handleReset() {
    setSelectedRole(null); setPrompt(''); setResult(null); setError(null)
    setDisplayScore(0); setScoreLanded(false); setFeedbackVisible(false)
    setShowCelebration(false); setReflection(''); setReflectionSaved(false)
  }

  const isSubmitDisabled = !selectedRole || !prompt.trim() || prompt.trim() === `You are a ${selectedRole}.` || isLoading
        ...
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', backdropFilter: 'blur(12px)' }}>
          ...
          {result === null && (
            <div className="flex flex-col gap-3">
              <p className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>Assign a Role to the AI</p>
                ...
                {ROLES.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      style={{ background: isSelected ? 'rgba(184,115,51,0.15)' : 'rgba(255,255,255,0.04)', border: `1.5px solid ${isSelected ? '#B87333' : 'rgba(255,255,255,0.1)'}`, color: isSelected ? '#B87333' : 'rgba(255,255,255,0.6)', cursor: 'pointer' }}
          ...
          {result === null && selectedRole && (
            <div className="flex flex-col gap-3">
              <label className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>
                Your Prompt <span style={{ color: 'rgba(0,255,136,0.4)' }}>— start with &ldquo;You are a {selectedRole}&rdquo;</span>
              </label>
              <textarea
                ...
                placeholder={`You are a ${selectedRole}. [Add your prompt here…]`}
                value={prompt}
          ...
          {result === null && (
            <button onClick={handleSubmit} disabled={isSubmitDisabled} className={`w-full py-4 font-bold text-sm tracking-wide transition-all duration-200 btn-primary${!isSubmitDisabled ? ' neon-pulse' : ''}`}>
              {isLoading ? <span className="flex items-center justify-center gap-2"><span className="inline-block w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: 'rgba(15,15,15,0.3)', borderTopColor: '#0F0F0F' }} />Scoring…</span> : !selectedRole ? 'Pick a role first' : 'Submit'}
            </button>
          )}
          ...
          {result !== null && (
            <div className="flex flex-col gap-5 animate-in fade-in duration-500">
              ...
              <div className="rounded-3xl p-6 flex flex-col gap-3 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0, background: 'rgba(184,115,51,0.04)', border: '1px solid rgba(184,115,51,0.15)' }}>
                ...
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>How did the {selectedRole} role shape the response you expected?</p>
