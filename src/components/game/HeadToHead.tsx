...
function makeWeakPrompt(challenge: string): string {
  const words = challenge.split(' ').slice(0, 5).join(' ')
  return `Write something about this. ${words}... make it good somehow. You know what I mean, right?`
}
...
export default function HeadToHead({
  ...
  async function handleSubmit() {
    if (!selection || !explanation.trim() || !improved.trim() || isLoading) return
  ...
  function handleReset() {
    setSelection(null); setExplanation(''); setImproved(''); setResult(null); setError(null)
    setDisplayScore(0); setScoreLanded(false); setFeedbackVisible(false)
    setShowCelebration(false); setReflection(''); setReflectionSaved(false)
    setUnlockedPartIds([])
  }
        ...
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: '#FAFAFA', border: '1.5px solid #E8E8E8',  }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {(['A', 'B'] as const).map(label => {
              const text = label === 'A' ? strongPrompt : weakPrompt
              ...
              return (
                <button
                  ...
                  style={{ background: isSelected ? 'rgba(74,144,226,0.08)' : 'rgba(0,0,0,0.02)', border: `1.5px solid ${isSelected ? '#4A90E2' : 'rgba(0,0,0,0.06)'}`, cursor: result === null ? 'pointer' : 'default' }}
                  aria-pressed={isSelected}
                  disabled={result !== null}
                >
                  <span className="text-xs font-mono font-bold" style={{ color: isSelected ? '#4A90E2' : '#999999' }}>Prompt {label}</span>
