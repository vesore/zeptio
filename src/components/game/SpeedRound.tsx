...
export default function SpeedRound({
  ...
  const timerColor = timeLeft <= 10 ? '#C84B1F' : timeLeft <= 20 ? '#B87333' : '#00FF88'
        ...
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', backdropFilter: 'blur(12px)' }}>
          ...
          <div className="flex items-center justify-between">
            ...
            {(phase === 'round1' || phase === 'round2' || phase === 'round3') && (
              <div className="flex flex-col items-center">
                ...
                <span className="text-xs" style={{ color: 'rgba(255,255,255,0.3)' }}>seconds</span>
              </div>
            )}
          </div>
          ...
          {phase === 'ready' && (
            <div className="flex flex-col gap-5">
              <div>
                <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-2" style={{ color: '#00FF88' }}>The Challenge</p>
              ...
              <div className="rounded-2xl p-4" style={{ background: 'rgba(184,115,51,0.06)', border: '1px solid rgba(184,115,51,0.15)' }}>
                <p className="text-sm font-mono" style={{ color: '#B87333' }}>3 prompts · 60 seconds · Best score counts</p>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.4)' }}>Write 3 different prompts for the same challenge. Press Next to advance between rounds. Best score used for XP and progression.</p>
              </div>
          ...
          {(phase === 'round1' || phase === 'round2' || phase === 'round3') && (
            <div className="flex flex-col gap-4">
              <div>
                <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-2" style={{ color: '#00FF88' }}>Prompt {currentRound} of 3</p>
              ...
              <div className="flex gap-2">
                {[1, 2, 3].map(n => (
                  <div
                    ...
                    style={{ background: n < currentRound ? '#00FF88' : n === currentRound ? 'rgba(0,255,136,0.4)' : 'rgba(255,255,255,0.1)' }}
                  />
                ))}
              </div>
          ...
          {phase === 'scoring' && (
            <div className="flex flex-col items-center gap-4 py-8">
              <div className="inline-block w-8 h-8 rounded-full border-2 animate-spin" style={{ borderColor: 'rgba(0,255,136,0.2)', borderTopColor: '#00FF88' }} />
              <p className="text-sm font-mono" style={{ color: 'rgba(255,255,255,0.5)' }}>Scoring all 3 prompts…</p>
            </div>
          )}
          ...
          {phase === 'done' && (
            <div className="flex flex-col gap-5 animate-in fade-in duration-500">
              ...
              <div className="flex flex-col gap-2">
                <p className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>Round Scores</p>
                {scores.map((s, i) => (
                  <div key={i} className="flex items-center gap-3 rounded-xl px-4 py-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <span className="text-xs font-mono" style={{ color: 'rgba(255,255,255,0.3)' }}>Round {i + 1}</span>
                    <div className="flex-1 text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>{prompts[i] ? `"${prompts[i].substring(0, 40)}${prompts[i].length > 40 ? '…' : ''}"` : '(skipped)'}</div>
                    {s ? (
                      <span className="text-xs font-mono font-bold" style={{ color: s.score >= 60 ? '#00FF88' : '#B87333' }}>{s.score}</span>
                    ) : (
              ...
              {bestResult ? (
                <>
                  <div className="flex flex-col items-center gap-2 py-4" aria-hidden="true">
                    <p className="text-xs font-mono uppercase tracking-widest mb-2" style={{ color: 'rgba(0,255,136,0.5)' }}>Best Score</p>
                    ...
                    {scoreLanded && <span className="rounded-full px-4 py-1.5 text-xs font-bold mt-1 animate-in fade-in duration-300" style={{ background: 'rgba(0,255,136,0.12)', color: '#00FF88', border: '1px solid rgba(0,255,136,0.25)' }}>+{bestResult.xp_earned} XP</span>}
                  </div>
                  ...
                  <div className="rounded-3xl p-6 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-3" style={{ color: '#00FF88' }}>Feedback — Best Prompt</p>
                    <p className="text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.75)' }}>{bestResult.feedback}</p>
                  </div>
                  ...
                  <div className="rounded-3xl p-6 flex flex-col gap-3 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0, background: 'rgba(184,115,51,0.04)', border: '1px solid rgba(184,115,51,0.15)' }}>
                    ...
                    <p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>What made your best prompt stronger than the others?</p>
                  ...
                  <div className="flex flex-col gap-3 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0 }}>
                    {bestResult.score >= 60 && nextLevelUrl ? (
                      ...
                      <>
                        {bestResult.score < 60 && <p className="text-xs text-center font-mono" style={{ color: 'rgba(255,255,255,0.35)' }}>Score 60 or higher to advance.</p>}
                ...
                <div className="flex flex-col gap-4 items-center py-4">
                  ...
                  <p className="text-sm font-mono" style={{ color: 'rgba(255,255,255,0.4)' }}>No prompts were submitted in time.</p>
                  <button onClick={handleReset} className="w-full py-4 font-bold text-sm tracking-wide btn-primary">Try Again</button>
                </div>
              )}
            </div>
