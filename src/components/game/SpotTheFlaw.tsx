...
interface FlawSegment {
  text: string
  isFlawed: boolean
  flawLabel: string
}

function buildFlawedPrompt(challenge: string): FlawSegment[] {
  const short = challenge.split(' ').slice(0, 6).join(' ')
  return [
    { text: 'Write something', isFlawed: true, flawLabel: 'Vague — no specific task' },
    { text: ` about ${short}`, isFlawed: false, flawLabel: '' },
    { text: ' maybe somehow and also do the opposite', isFlawed: true, flawLabel: 'Contradiction — unclear direction' },
    { text: '. Be both extremely formal and super casual', isFlawed: true, flawLabel: 'Impossible tone' },
    { text: ' in exactly 3 words but make it very comprehensive and detailed', isFlawed: true, flawLabel: 'Impossible constraint' },
    { text: '. You know what I mean', isFlawed: true, flawLabel: 'Implicit assumption — no real instruction' },
  ]
}
...
export default function SpotTheFlaw({
  ...
  const segments = buildFlawedPrompt(levelConfig.challenge)
  const flawedSegments = segments.filter(s => s.isFlawed)
  ...
  async function handleSubmit() {
    if (selected.size === 0 || !rewrite.trim() || isLoading) return
    ...
    const selectedFlaws = Array.from(selected)
      .map(i => segments[i]?.flawLabel)
      .filter(Boolean)
      .join(', ')
  ...
  const correctlyIdentified = Array.from(selected).filter(i => correctFlawIndices.has(i)).length
      ...
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', backdropFilter: 'blur(12px)' }}>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-semibold uppercase tracking-widest px-3 py-1 rounded-full" style={{ background: 'rgba(200,75,31,0.1)', color: '#C84B1F', border: '1px solid rgba(200,75,31,0.2)' }}>
              Spot the Flaw
            </span>
          </div>

          <div>
            <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-1" style={{ color: '#00FF88' }}>Step 1 — Click the flawed parts</p>
            <p className="text-xs mb-4" style={{ color: 'rgba(255,255,255,0.4)' }}>This prompt has {flawedSegments.length} problems. Click each flawed segment to mark it.</p>

            {/* Flawed prompt with clickable segments */}
            <div
              className="rounded-2xl p-5 leading-loose text-base font-mono"
              style={{ background: 'rgba(200,75,31,0.05)', border: '1px solid rgba(200,75,31,0.15)' }}
            >
              {segments.map((seg, i) => {
                const isMarked = selected.has(i)
                const isClickable = seg.isFlawed && result === null
                return (
                  <span
                    key={i}
                    onClick={() => isClickable && toggleSegment(i)}
                    title={result !== null && seg.isFlawed ? seg.flawLabel : undefined}
            ...
            {selected.size > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {Array.from(selected).map(i => segments[i] && (
                  <span
                    key={i}
                    className="text-xs font-mono px-3 py-1 rounded-full cursor-pointer animate-in fade-in duration-200"
                    onClick={() => toggleSegment(i)}
                    style={{ background: 'rgba(200,75,31,0.15)', color: '#C84B1F', border: '1px solid rgba(200,75,31,0.3)' }}
                  >
                    {segments[i].flawLabel || `Segment ${i + 1}`} ×
                  </span>
                ))}
              </div>
            )}
          ...
          {/* Reveal correct flaws after scoring */}
          {result !== null && feedbackVisible && (
            <div className="rounded-2xl p-4 animate-in fade-in duration-500" style={{ background: 'rgba(0,255,136,0.06)', border: '1px solid rgba(0,255,136,0.15)' }}>
              <p className="text-xs font-mono mb-2" style={{ color: '#00FF88' }}>You found {correctlyIdentified} of {flawedSegments.length} flaws correctly.</p>
              <div className="flex flex-col gap-1">
                {flawedSegments.map((s, i) => (
                  <p key={i} className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>• {s.flawLabel}</p>
                ))}
              </div>
            </div>
          )}
          ...
          {result === null && (
            <div className="flex flex-col gap-3">
              <label className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>
                Step 2 — Write the fixed version
              </label>
              <textarea
                ...
                placeholder="Rewrite the prompt without the flaws you identified…"
                value={rewrite}
          ...
          {result === null && (
            <button onClick={handleSubmit} disabled={isSubmitDisabled} className={`w-full py-4 font-bold text-sm tracking-wide transition-all duration-200 btn-primary${!isSubmitDisabled ? ' neon-pulse' : ''}`}>
              ...
              ) : selected.size === 0 ? 'Mark at least one flaw' : !rewrite.trim() ? 'Write your fixed version' : 'Submit'}
            </button>
          )}
          ...
          {result !== null && (
            <div className="flex flex-col gap-5 animate-in fade-in duration-500">
              ...
              <div className="rounded-3xl p-6 flex flex-col gap-3 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0, background: 'rgba(184,115,51,0.04)', border: '1px solid rgba(184,115,51,0.15)' }}>
                ...
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>Which type of flaw is hardest to spot in your own writing?</p>
