      ...
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: '#FAFAFA', border: '1.5px solid #E8E8E8',  }}>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono font-semibold uppercase tracking-widest px-3 py-1 rounded-full" style={{ background: 'rgba(226,74,74,0.1)', color: '#E24A4A', border: '1px solid rgba(226,74,74,0.2)' }}>
              Spot the Flaw
            </span>
          </div>

          <div>
            <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-1" style={{ color: '#4A90E2' }}>Step 1 — Click the flawed parts</p>
            <p className="text-xs mb-4" style={{ color: '#888888' }}>This prompt has {flawedSegments.length} problems. Click each flawed segment to mark it.</p>

            {/* Flawed prompt with clickable segments */}
            <div
              className="rounded-2xl p-5 leading-loose text-base font-mono"
              style={{ background: 'rgba(226,74,74,0.05)', border: '1px solid rgba(226,74,74,0.15)' }}
            >
              {segments.map((seg, i) => {
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {Array.from(selected).map(i => segments[i] && (
                  <span
                    ...
                    style={{ background: 'rgba(226,74,74,0.15)', color: '#E24A4A', border: '1px solid rgba(226,74,74,0.3)' }}
                  >
          ...
          {result !== null && feedbackVisible && (
            <div className="rounded-2xl p-4 animate-in fade-in duration-500" style={{ background: 'rgba(74,144,226,0.06)', border: '1px solid rgba(74,144,226,0.15)' }}>
              <p className="text-xs font-mono mb-2" style={{ color: '#4A90E2' }}>You found {correctlyIdentified} of {flawedSegments.length} flaws correctly.</p>
              <div className="flex flex-col gap-1">
                {flawedSegments.map((s, i) => (
                  <p key={i} className="text-xs" style={{ color: '#666666' }}>• {s.flawLabel}</p>
                ))}
              </div>
            </div>
          )}
