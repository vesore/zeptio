...
const FORMAT_EXAMPLES: Record<Format, React.ReactNode> = {
  'Bullet Points': (
    <div className="flex flex-col gap-1">
      <p className="text-xs font-mono" style={{ color: 'rgba(255,255,255,0.5)' }}>• First key point</p>
      <p className="text-xs font-mono" style={{ color: 'rgba(255,255,255,0.5)' }}>• Second key point</p>
      <p className="text-xs font-mono" style={{ color: 'rgba(255,255,255,0.5)' }}>• Third key point</p>
    </div>
  ),
  'Paragraph': (
  ...
  'Table': (
    <div className="overflow-x-auto">
      <table className="w-full text-xs font-mono" style={{ color: 'rgba(255,255,255,0.5)', borderCollapse: 'collapse' }}>
        <thead><tr><th className="text-left pr-4 pb-1" style={{ color: 'rgba(0,255,136,0.5)' }}>Column A</th><th className="text-left pb-1" style={{ color: 'rgba(0,255,136,0.5)' }}>Column B</th></tr></thead>
    ...
    <div className="min-h-screen w-full max-w-full overflow-x-hidden flex items-center justify-center" style={{ background: '#0F0F0F' }}>
      ...
      <div className="w-full max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="w-full rounded-3xl p-5 sm:p-8 flex flex-col gap-5 sm:gap-6" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', backdropFilter: 'blur(12px)' }}>
          ...
          <div>
            <p className="text-xs font-mono font-semibold uppercase tracking-widest mb-2" style={{ color: '#00FF88' }}>Content Request</p>
          ...
          {result === null && (
            <div className="flex flex-col gap-3">
              <p className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>Choose a Format</p>
          ...
          {result === null && selectedFormat && (
            <div className="flex flex-col gap-3">
              <label className="text-xs font-mono font-semibold uppercase tracking-widest" style={{ color: '#00FF88' }}>
                Your Prompt <span style={{ color: 'rgba(0,255,136,0.4)' }}>— must request {selectedFormat}</span>
              </label>
              <textarea
          ...
          {result === null && (
            <button onClick={handleSubmit} disabled={isSubmitDisabled} className={`w-full py-4 font-bold text-sm tracking-wide transition-all duration-200 btn-primary${!isSubmitDisabled ? ' neon-pulse' : ''}`}>
              {isLoading ? <span className="flex items-center justify-center gap-2"><span className="inline-block w-4 h-4 rounded-full border-2 animate-spin" style={{ borderColor: 'rgba(15,15,15,0.3)', borderTopColor: '#0F0F0F' }} />Scoring…</span> : !selectedFormat ? 'Pick a format first' : 'Submit'}
            </button>
          )}
          ...
          {result !== null && (
            <div className="flex flex-col gap-5 animate-in fade-in duration-500">
              ...
              <div className="rounded-3xl p-6 flex flex-col gap-3 transition-opacity duration-500" style={{ opacity: feedbackVisible ? 1 : 0, background: 'rgba(184,115,51,0.04)', border: '1px solid rgba(184,115,51,0.15)' }}>
                ...
                <p className="text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>When would {selectedFormat?.toLowerCase()} be the wrong choice?</p>
