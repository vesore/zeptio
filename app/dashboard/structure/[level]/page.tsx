...
function isInfiniteLevelUnlocked(levelIndex: number, bestScores: Map<number, number>, infiniteScores: Map<number, number>): boolean {
  if (levelIndex === STRUCTURE_LEVELS.length + 1) {
    const lastId = STRUCTURE_LEVELS[STRUCTURE_LEVELS.length - 1].id
    return (bestScores.get(lastId) ?? 0) >= 60
  }
  const prevInfiniteId = getInfiniteLevelId('structure', levelIndex - 1)
...
export default async function StructureLevelPage({ params }: Props) {
  ...
  const isInfinite = levelIndex > STRUCTURE_LEVELS.length
  ...
  const [{ data: scoreRows }, { data: allScoreRows }, profileResult] = await Promise.all([
    ...
    isInfinite
      ? supabase.from('xp_ledger').select('level_id, score').eq('user_id', user.id).eq('world', 'structure')
  ...
  if (!isInfinite) {
    if (levelIndex > STRUCTURE_LEVELS.length) notFound()
    ...
    const nextLevelUrl = `/dashboard/structure/${levelIndex + 1}`
      ...
      <div className="w-full max-w-2xl mx-auto px-3 sm:px-6 pt-3 sm:pt-6 mt-0 sm:mt-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            ...
            style={{ background: 'rgba(74,226,122,0.06)', color: 'rgba(74,226,122,0.5)' }}
          >
            ∞ Level {levelIndex}
          </span>
        </div>
